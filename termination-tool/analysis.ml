...
let processFile (cil : C.file) =
  feature.C.fd_doit cil
...
let _ =
  ...
  Arg.parse (Arg.align []) Ciloptions.recordFile usageMsg;
  Cil.initCIL ();
  Ciloptions.fileNames := List.rev !Ciloptions.fileNames;
  let files = Util.list_map parseFile !Ciloptions.fileNames in
  let one =
    match files with
      [one] -> one
    | [] -> assert false
